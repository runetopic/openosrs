		...
		if (var1 != null) {
			...
			if (var2.hasCleanName()) {
				if (this.friendsList.removeByUsername(var2)) {
					...
					PacketBufferNode var3 = SecureUrlRequester.getPacketBufferNode(ClientPacket.field3199, Client.packetWriter.isaacCipher);
					var3.packetBuffer.writeByte(ClanChannel.stringCp1252NullTerminatedByteSize(var1));
					var3.packetBuffer.writeStringCp1252NullTerminated(var1);
					Client.packetWriter.addNode(var3);
				}
		...
		if (var1 != null) {
			...
			if (var3.hasCleanName()) {
				if (this.ignoreList.removeByUsername(var3)) {
					JagexCache.method4214();
					if (var2) {
						PacketBufferNode var4 = SecureUrlRequester.getPacketBufferNode(ClientPacket.field3133, Client.packetWriter.isaacCipher);
