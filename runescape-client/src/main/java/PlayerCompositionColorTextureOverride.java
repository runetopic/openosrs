...
public class PlayerCompositionColorTextureOverride {
   ...
   static final void updatePendingSpawn(int plane, int x, int y, int type, int id, int var5, int orientation, int var7, int delay, int hitpoints) {
      ...
      for(PendingSpawn var11 = (PendingSpawn)Client.pendingSpawns.last(); var11 != null; var11 = (PendingSpawn)Client.pendingSpawns.previous()) {
         if (plane == var11.plane && var11.x == x && y == var11.y && type == var11.type) {
            var10 = var11;
            break;
         }
      }
